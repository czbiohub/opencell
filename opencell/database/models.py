...
class MicroscopyFOVResults(Base):
    '''
    Various results, as arbitrary JSON objects,
    generated by parsing the TIFF metadata or processing the z-stack itself

    Examples: the metadata generated by RawFOVProcessor.process_raw_tiff,
    or the features extracted from the DAPI z-projection used to predict FOV scores
    '''

    __tablename__ = 'microscopy_fov_results'
    ...
    # the type/kind of the result - 'tiff_metadara', 'fov_features', etc
    # (eventually, this should be changed to an enum)
    kind = db.Column(db.String)

    # the results themselves
    results = db.Column(db.types.JSON)

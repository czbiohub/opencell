...
class CellLine(Base):
    ...
    # each crispr_design should appear only once in a given electroporation,
    # because plates are electroporated one at a time
    # (note that unique constraints 'ignore' nulls)
    __table_args__ = (
        db.UniqueConstraint(electroporation_id, crispr_design_id),
    )
...
class Electroporation(Base):
    ...
    # the date on which the electroporation was performed
    ...
    # (using the same progenitor cell line)
    __table_args__ = (
        db.UniqueConstraint(progenitor_cell_line_id, plate_design_id, date_performed),
    )
...
class MassSpecClusterHeatmap(Base):
    ...
    hit = db.orm.relationship("MassSpecHit")

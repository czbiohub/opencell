...
class MicroscopyFOV(Base):
    ...
    # one-to-many relationship with FOV metadata and features
    fov_metadata = db.orm.relationship('MicroscopyFOVMetadata', back_populates='fov')
    fov_features = db.orm.relationship('MicroscopyFOVFeatures', back_populates='fov')
...
class MicroscopyFOVMetadata(Base):
    '''
    Metadata generated by parsing the TIFF metadata or processing the z-stack itself
    For now, we use JSON because the metadata properties are not settled
    '''

    __tablename__ = 'microscopy_fov_metadata'

    fov_id = db.Column(db.Integer, db.ForeignKey('microscopy_fov.id'), primary_key=True)
    fov = db.orm.relationship('MicroscopyFOV', back_populates='fov_metadata', uselist=False)

    timestamp = db.Column(db.DateTime(timezone=True), server_default=db.sql.func.now())

    # metadata generated by RawFOVProcessor.process_raw_tiff
    tiff_metadata = db.Column(db.types.JSON)
    

class MicroscopyFOVFeatures(Base):
    '''
    Features extracted from a raw z-stack
    '''

    __tablename__ = 'microscopy_fov_features'

    fov_id = db.Column(db.Integer, db.ForeignKey('microscopy_fov.id'), primary_key=True)
    fov = db.orm.relationship('MicroscopyFOV', back_populates='fov_features', uselist=False)

    timestamp = db.Column(db.DateTime(timezone=True), server_default=db.sql.func.now())

    # features extracted from the DAPI z-projection (used for predicting FOV scores)
    score_features = db.Column(db.types.JSON)
